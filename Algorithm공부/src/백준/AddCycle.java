...
/**
 * 문제: 0보다 크거나 같고, 99보다 작거나 같은 정수가 주어질 때 다음과 같은 연산을 할 수 있다. 먼저 주어진 수가 10보다 작다면
 * 앞에 0을 붙여 두 자리 수로 만들고, 각 자리의 숫자를 더한다. 그 다음, 주어진 수의 가장 오른쪽 자리 수와 앞에서 구한 합의 가장
...
public class AddCycle {
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			if (value >= 0 && value <= 99) {
				// 10으로 나눈 나머지에 10을 곱하면 십의 자리수가 됨.
				// value에서 나누기 10을 하면 value가 한 자리 수일경우 0 이되고 그이외는 십의 자리수가 그대로 유지됨.
